...
internal object ContractTermSerializer : KSerializer<Contract.Term> {
  ...
  private fun serializeInternal(encoder: CompositeEncoder, `value`: Contract.Term) {
    ...
    when (val choice = value.topic) {
      null -> {}
      is CodeableConcept -> {
        encoder.encodeSerializableElement(descriptor, 7, Hoisted.topicCodeableConceptSer, choice)
      }
      is Reference -> {
        encoder.encodeSerializableElement(descriptor, 8, Hoisted.topicReferenceSer, choice)
      }
    }
    (value.type)?.let {
    ...
    return Contract.Term.Offer(
      ...
      linkId =
        (kotlin.collections.List(maxOf(linkId?.size ?: 0, _linkId?.size ?: 0)) { index ->
          R4bString.of(linkId?.getOrNull(index)?.let { it }, _linkId?.getOrNull(index))!!
        }),
      securityLabelNumber =
...
internal object ContractTermAssetValuedItemSerializer :
  ...
  private fun serializeInternal(
    ...
    when (val choice = value.entity) {
      null -> {}
      is CodeableConcept -> {
        encoder.encodeSerializableElement(descriptor, 3, Hoisted.entityCodeableConceptSer, choice)
      }
      is Reference -> {
        encoder.encodeSerializableElement(descriptor, 4, Hoisted.entityReferenceSer, choice)
      }
    }
    (value.identifier)?.let {
...
internal object ContractTermActionSerializer : KSerializer<Contract.Term.Action> {
    ...
    return Contract.Term.Action(
      ...
      reason =
        (kotlin.collections.List(maxOf(reason?.size ?: 0, _reason?.size ?: 0)) { index ->
          R4bString.of(reason?.getOrNull(index)?.let { it }, _reason?.getOrNull(index))!!
        }),
      reasonLinkId =
        (kotlin.collections.List(maxOf(reasonLinkId?.size ?: 0, _reasonLinkId?.size ?: 0)) { index
          ->
          R4bString.of(
            reasonLinkId?.getOrNull(index)?.let { it },
            _reasonLinkId?.getOrNull(index),
          )!!
        }),
      note = note ?: listOf(),
      securityLabelNumber =
  ...
  private fun serializeInternal(encoder: CompositeEncoder, `value`: Contract.Term.Action) {
    ...
    when (val choice = value.occurrence) {
      ...
      is Period -> {
        encoder.encodeSerializableElement(descriptor, 16, Hoisted.occurrencePeriodSer, choice)
      }
      is Timing -> {
        encoder.encodeSerializableElement(descriptor, 17, Hoisted.occurrenceTimingSer, choice)
      }
    }
