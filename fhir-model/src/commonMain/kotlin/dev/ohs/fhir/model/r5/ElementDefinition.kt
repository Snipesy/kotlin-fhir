...
import dev.ohs.fhir.model.r5.serializers.ElementDefinitionBaseSerializer
...
import dev.ohs.fhir.model.r5.serializers.ElementDefinitionTypeSerializer
...
public data class ElementDefinition(
   ...
   *
   * A FHIR choice type — one of: [Date] | [DateTime] | [Decimal] | [Instant] | [Integer] |
   * [Integer64] | [PositiveInt] | [Quantity] | [Time] | [UnsignedInt]
   */
   ...
   */
  public val mustHaveValue: Boolean? = null,
  /**
   ...
   */
  public val valueAlternatives: List<Canonical> = listOf(),
  /**
  ...
  public data class Example(
    ...
    public class Builder(
       ...
       * [Availability] | [Base64Binary] | [Boolean] | [Canonical] | [Code] | [CodeableConcept] |
       * [CodeableReference] | [Coding] | [ContactDetail] | [ContactPoint] | [Count] |
       * [DataRequirement] | [Date] | [DateTime] | [Decimal] | [Distance] | [Dosage] | [Duration] |
       * [Expression] | [ExtendedContactDetail] | [HumanName] | [Id] | [Identifier] | [Instant] |
       * [Integer] | [Integer64] | [Markdown] | [Meta] | [Money] | [Oid] | [ParameterDefinition] |
  ...
  public data class Constraint(
    ...
    public class Builder(
       ...
       * To be used if the reason for the constraint might not be intuitive to all implementers.
       */
      public var requirements: Markdown.Builder? = null
       ...
       * that the profile explicitly prohibits.
       */
      public var suppress: Boolean.Builder? = null
  ...
  public data class Binding(
    ...
    public data class Additional(
       ...
       */
      public val purpose: Enumeration<AdditionalBindingPurposeVS>,
       ...
       */
      public val usage: List<UsageContext> = listOf(),
      /**
      ...
      public class Builder(
        ...
        /** Concise documentation - for summary tables. */
        public var shortDoco: String.Builder? = null

        /**
         * Qualifies the usage of the binding. Typically bindings are qualified by jurisdiction, but
         * they may also be qualified by gender, workflow status, clinical domain etc. The
         * information to decide whether a usege context applies is usually outside the resource,
         * determined by context, and this might present challenges for validation tooling.
         *
         * This specification does not define exactly how the context is determined for the usage;
         * typically, this will be described in implementation guides.
         */
        public var usage: MutableList<UsageContext.Builder> = mutableListOf()
         ...
         * that one of the repeats must conform to.
         */
        public var any: Boolean.Builder? = null
    ...
    public class Builder(
      ...
      /**
       * Additional bindings that help applications implementing this element. Additional bindings
       * do not replace the main binding but provide more information and/or context.
       */
      public var additional: MutableList<Additional.Builder> = mutableListOf()
  ...
  public open class Builder(
     ...
     * from the normal case. No extensions are allowed on elements with a representation of
     * 'xmlAttr', no matter what FHIR serialization format is used.
     *
     * In resources, this is rarely used except for special cases where the representation deviates
     * from the normal, and can only be done in the base standard (and profiles must reproduce what
     ...
     * the element definition is not the original definition of an element - e.g. either in a
     ...
     * 3. If an array: it must match (recursively) the pattern value
     *
     * If a pattern[x] is declared on a repeating element, the pattern applies to all repetitions.
     * If the desire is for a pattern to apply to only one element or a subset of elements, slicing
     * must be used. See [Examples of Patterns](elementdefinition-examples.html#pattern-examples)
     * for examples of pattern usage and the effect it will have.
     *
     * Mostly used for fixing values of CodeableConcept. In general, pattern[x] is not intended for
     * use with primitive types, where is has the same meaning as fixed[x].
     *
     ...
     * as a canonical minimum - e.g. you cannot provide 100mg if the minimum value is 10g.
     *
     * A FHIR choice type — one of: [Date] | [DateTime] | [Decimal] | [Instant] | [Integer] |
     * [Integer64] | [PositiveInt] | [Quantity] | [Time] | [UnsignedInt]
     */
     ...
     * to determine the maximum allowable value. A maximum value for a Quantity is interpreted as a
     * canonical maximum - e.g. you cannot provide 10g if the maximum value is 50mg.
     *
     * A FHIR choice type — one of: [Date] | [DateTime] | [Decimal] | [Instant] | [Integer] |
     * [Integer64] | [PositiveInt] | [Quantity] | [Time] | [UnsignedInt]
     ...
     * and
     * [http://hl7.org/fhir/StructureDefinition/cqf-expression](http://hl7.org/fhir/extensions/StructureDefinition-cqf-expression.html).
     * Note that this element has no meaning when a value is provided, and extensions can be
     * provided in addition to a value.
     */
    public open var mustHaveValue: Boolean.Builder? = null
     ...
     * allowed in place of the value. Note that this element has no impact if the value is present.
     */
    public open var valueAlternatives: MutableList<Canonical.Builder> = mutableListOf()

    /**
     * If true, implementations that produce or consume resources SHALL provide "support" for the
     * element in some meaningful way. Note that this is being phased out and replaced by
     * obligations (see below). If false, the element may be ignored and not supported. If false,
     * whether to populate or use the data element in any way is at the discretion of the
     * implementation.
     *
     * "Something meaningful" is context dependent and impossible to describe in the base FHIR
     * specification. For this reason, the mustSupport flag is never set to true by the FHIR
     * specification itself - it is only set to true in profiles. A profile on a type can always
     * make mustSupport = true if it is false in the base type but cannot make mustSupport = false
     * if it is true in the base type. This is done in
     * [Resource Profiles](profiling.html#mustsupport), where the profile labels an element as
     * mustSupport=true. When a profile does this, it SHALL also make clear exactly what kind of
     * "support" is required, as this can mean many things. Note that an element that has the
     * property IsModifier is not necessarily a "key" element (e.g. one of the important elements to
     * make use of the resource), nor is it automatically mustSupport - however both of these things
     * are more likely to be true for IsModifier elements than for other elements.
     */
     ...
     * ignored by systems: they SHALL either recognize the element and process it, and/or a
     * pre-determination has been made that it is not relevant to their particular system. When used
     * on the root element in an extension definition, this indicates whether or not the extension
     ...
     * profiles (except in the special case of the defining a new extension). An element/extension
     * that has isModifier=true SHOULD also have a minimum cardinality of 1, so that there is no
     * lack of clarity about what to do if it is missing. If it can be missing, the definition SHALL
     * make the meaning of a missing element clear.
     */
     ...
     * summarized resources for optimal performance. When a request is made with _summary=true,
     * serializers only include elements marked as 'isSummary = true'. With a few exceptions (listed
     * below), all datatype properties are included in the summary form. In resource and datatype
     * definitions, if an element has a descendant marked as isSummary=true or if it satisfies both
     * of the following conditions, it must be marked as isSummary=true:
     * * is at the root or has a parent that is 'mustSupport'
     * * has a minimum cardinality of 1 or is a modifier element
     *
     * The following datatype properties are exceptions, and are not marked as isSummary:
