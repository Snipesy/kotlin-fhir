...
internal object CoverageEligibilityResponseInsuranceItemBenefitSerializer :
  ...
  private fun serializeInternal(
    ...
    when (val choice = value.used) {
      ...
      is Money -> {
        encoder.encodeSerializableElement(descriptor, 13, Hoisted.allowedMoneySer, choice)
      }
    }
  }
...
internal object CoverageEligibilityResponseErrorSerializer :
  ...
  private fun serializeInternal(
    ...
    (value.expression.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(descriptor, 4, Hoisted.expressionSer, it)
    }
    (value.expression.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(descriptor, 5, Hoisted.expressionSer2, it)
    }
  }
  ...
  private object Hoisted {
    ...
    public val expressionSerInner2: KSerializer<Element> = Element.serializer()

    public val expressionSer2: KSerializer<List<Element?>> =
      ListSerializer((Hoisted.expressionSerInner2).nullable)
  }
}
...
internal object CoverageEligibilityResponseSerializer : KSerializer<CoverageEligibilityResponse> {
    ...
    while (true) {
      ...
      when (i - descriptorOffset) {
        ...
        31 ->
          _preAuthRef =
            decoder.decodeNullableSerializableElement(descriptor, i, Hoisted.implicitRulesSer, null)
        32 -> form = decoder.decodeNullableSerializableElement(descriptor, i, Hoisted.formSer, null)
        33 ->
  ...
  internal fun serializeInternal(
    ...
    (value.form)?.let {
      encoder.encodeSerializableElement(descriptor, 32 + descriptorOffset, Hoisted.formSer, it)
    }
  ...
  private object Hoisted {
    ...
    public val eventSerInner: KSerializer<CoverageEligibilityResponse.Event> =
      CoverageEligibilityResponse.Event.serializer()

    public val eventSer: KSerializer<List<CoverageEligibilityResponse.Event>> =
