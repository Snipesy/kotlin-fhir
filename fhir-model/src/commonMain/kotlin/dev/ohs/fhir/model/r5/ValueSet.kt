...
public data class ValueSet(
  ...
  public data class Compose(
    ...
    public data class Include(
      ...
      public class Builder() {
         ...
         *
         * The value set URI is either a logical reference to a defined value set such as a
         * [SNOMED CT reference set](https://terminology.hl7.org/SNOMEDCT.html), or a direct
         * reference to a value set definition using ValueSet.url. The reference might not refer to
         * an actual FHIR ValueSet resource; in this case, whatever is referred to is an implicit
    ...
    public class Builder(
      ...
      /**
       * A property to return in the expansion, if the client doesn't ask for any particular
       * properties. May be either a code from the code system definition (convenient) or a the
       * formal URI that refers to the property. The special value '*' means all properties known to
       * the server.
       *
       * Note that property names can clash, so using a URI is recommended.
       */
  ...
  public data class Expansion(
    ...
    public class Builder(
       ...
       * structure, and only the server understands them.
       *
       * Clients SHOULD use the next link, if present, to page through expansion results in
       * preference to using the offset and count parameters. Due to the optional nature of the next
       * link, its absence does not necessarily indicate that it is the last page of results.
       * Instead, as the offset and count parameters SHALL be populated when paging, clients can
       * reliably use the count/offset parameters to determine whether the whole expansion is
       * returned.
       */
      public var next: Uri.Builder? = null
  ...
  public data class Scope(
    ...
    public class Builder() {
      ...
      /** Criteria describing which concepts or codes should be included and why. */
      public var inclusionCriteria: String.Builder? = null
  ...
  public class Builder(
     ...
     *
     * Usually an organization but may be an individual. This is not necessarily the same individual
     * or organization that developed and initially authored the content. The publisher is the
     * primary point of contact for questions or issues with the value set. This item SHOULD be
     ...
     * The period during which the ValueSet content was or is planned to be in active use.
     *
     * The effective period for a ValueSet determines when the content is applicable for usage and
     * is independent of publication and review dates. For example, a valueset intended to be used
     ...
     * categorization as well as keywords for the ValueSet that can be useful for filtering and
     * searching.
     *
     * DEPRECATION NOTE: For consistency, implementations are encouraged to migrate to using the new
     * 'topic' code in the useContext element. (I.e. useContext.code indicating
     ...
     * review of some aspect of the ValueSet.
     *
     * See guidance around (not) making local changes to elements
     * [here](canonicalresource.html#localization).
     */
     ...
     * further clarify the text in ValueSet.description.
     */
    public var scope: Scope.Builder? = null
