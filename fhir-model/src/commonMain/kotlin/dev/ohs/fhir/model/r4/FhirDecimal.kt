...
private constructor(
  ...
  /** The exact lexical form to emit on the wire; always matches [WIRE_REGEX]. */
  ...
  public companion object : BigNumber.Creator<FhirDecimal> {
    /** The FHIR `decimal` lexical pattern this FHIR version permits on the wire. */
    public val WIRE_REGEX: Regex = Regex("-?(0|[1-9][0-9]*)(\\.[0-9]+)?([eE][+-]?[0-9]+)?")
     ...
     * if [string] is not a valid FHIR decimal.
     */
    public fun fromString(string: String): FhirDecimal {
      require(string.matches(WIRE_REGEX)) { """Invalid FHIR decimal: $string""" }
      return FhirDecimal(BigDecimal.parseString(string), string)
    }

    /**
     * Wraps a [BigDecimal], deriving a canonical wire form that satisfies [WIRE_REGEX] (plain
     * notation when it fits, otherwise scientific). Throws if the value cannot be represented as a
     * FHIR decimal for this version (e.g. too many digits).
     */
    public fun fromBigDecimal(bigDecimal: BigDecimal): FhirDecimal =
      FhirDecimal(bigDecimal, canonicalWire(bigDecimal))

    private fun canonicalWire(bigDecimal: BigDecimal): String {
    ...
    override fun parseString(string: String, base: Int): FhirDecimal =
      if (base == 10 && string.matches(WIRE_REGEX)) fromString(string)
      else fromBigDecimal(BigDecimal.parseString(string, base))
