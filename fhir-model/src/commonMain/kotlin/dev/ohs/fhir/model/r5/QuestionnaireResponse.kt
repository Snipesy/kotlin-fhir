...
public data class QuestionnaireResponse(
  ...
  public data class Item(
    ...
    public data class Answer(
      ...
      public class Builder(
         ...
         * Questionnaire.item.type in the corresponding Questionnaire. Note that a question is
         * answered using one of the possible choices defined with answerOption, answerValueSet or
         * some other means and the answer has a complex data type, all elements within the answer
         * in the QuestionnaireResponse **SHOULD** match the elements defined corresponding choice
         * value in the Questionnaire. However, it is possible that not all elements will be
         * propagated. Also, some systems might use language translations resulting in different
         * displays. Comparison of value to the values defined in the Questionnaire (whether by
         * answerOption, answerValueSet or answerExpression) **SHALL NOT** pay attention to
         * Coding.display, Reference.display, Quantity.unit unless those are the only elements
  ...
  public class Builder(
     ...
     *
     * If a QuestionnaireResponse references a Questionnaire that can be resolved, then the
     * QuestionnaireResponse structure must be consistent with the Questionnaire (i.e. questions
     * must be organized into the same groups, nested questions must still be nested, etc.). It is
     * possible to have a QuestionnaireResponse whose 'questionnaire' element does not resolve. It
     * is also possible for the questionnaire element to not have a value but only extensions (e.g.
     * conveying the title or identifier for the questionnaire). This may happen for legacy data. If
     * there is no formally defined Questionnaire, it is undefined what the 'correct' values for the
     * linkId elements should be and it is possible that linkIds might be inconsistent for
     * QuestionnaireResponses for the same form if captured by distinct systems.
     */
    public var questionnaire: Canonical.Builder,
    /**
     * The current state of the questionnaire response.
     *
     * Unknown does not represent "other" - one of the defined statuses must apply. Unknown is used
    ...
    /**
     * A plan, proposal or order that is fulfilled in whole or in part by this questionnaire
     * response. For example, a ServiceRequest seeking an intake assessment or a decision support
     ...
     * Not to be used to link an questionnaire response to an Encounter - use 'context' for that.
     *
     * Composition of questionnaire responses will be handled using the Assemble operation defined
     * in the SDC IG. For relationships to referrals, and other types of requests, use basedOn.
     */
     ...
     * Mapping a subject's answers to multiple choice options and determining what to put in the
     * textual answer is a matter of interpretation. Authoring by device would indicate that some
     * portion of the questionnaire had been auto-populated. Device should only be used if it
     * directly determined the answers, not if it was merely used as a capture tool to record
     * answers provided by others. In the latter case, information about the physical device,
     * software, etc. would be captured using Provenance.
     */
