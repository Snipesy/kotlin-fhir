...
internal object AgeSerializer : KSerializer<Age> {
  ...
  private fun deserializeInternal(decoder: CompositeDecoder): Age {
    ...
    while (true) {
      when (val i = decoder.decodeElementIndex(descriptor)) {
        ...
        else -> throw SerializationException("Unexpected index decoding Age: " + i)
      }
    }
    return Age(
      id = id,
  ...
  private fun serializeInternal(encoder: CompositeEncoder, `value`: Age) {
