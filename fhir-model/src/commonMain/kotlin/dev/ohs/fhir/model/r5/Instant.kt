...
import dev.ohs.fhir.model.r5.serializers.FhirDateTimeSerializer
import dev.ohs.fhir.model.r5.serializers.InstantSerializer
...
public data class Instant(
...
) : PrimitiveType(), FhirChoiceParticipants.InstantChoices {
  ...
  public open class Builder() {
    ...
    public open fun build(): Instant =
      Instant(id = id, extension = extension.map { it.build() }, `value` = `value`)
  }

  public companion object {
    public fun of(`value`: FhirDateTime?, element: Element?): Instant? =
      if (value != null || element?.id != null || element?.extension?.isEmpty() == false) {
        Instant(element?.id, element?.extension ?: mutableListOf(), value)
      } else {
        null
      }
  }
