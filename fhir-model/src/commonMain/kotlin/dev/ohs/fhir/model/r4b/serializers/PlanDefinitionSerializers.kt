...
internal object PlanDefinitionActionSerializer : KSerializer<PlanDefinition.Action> {
  ...
  private fun serializeInternal(encoder: CompositeEncoder, `value`: PlanDefinition.Action) {
    ...
    if (value.trigger.isNotEmpty())
      encoder.encodeSerializableElement(descriptor, 22, Hoisted.triggerSer, value.trigger)
    if (value.condition.isNotEmpty())
      encoder.encodeSerializableElement(descriptor, 23, Hoisted.conditionSer, value.condition)
    if (value.input.isNotEmpty())
      encoder.encodeSerializableElement(descriptor, 24, Hoisted.inputSer, value.input)
    if (value.output.isNotEmpty())
      encoder.encodeSerializableElement(descriptor, 25, Hoisted.inputSer, value.output)
    ...
    (value.transform?.toElement())?.let {
      encoder.encodeSerializableElement(descriptor, 51, Hoisted.prefixSer, it)
    }
...
internal object PlanDefinitionSerializer : KSerializer<PlanDefinition> {
  ...
  internal fun deserializeInternal(
    ...
    while (true) {
      ...
      when (i - descriptorOffset) {
        ...
        58 -> goal = decoder.decodeNullableSerializableElement(descriptor, i, Hoisted.goalSer, null)
        59 ->
          action = decoder.decodeNullableSerializableElement(descriptor, i, Hoisted.actionSer, null)
  ...
  internal fun serializeInternal(
    ...
    (value.library.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(descriptor, 56 + descriptorOffset, Hoisted.librarySer, it)
    }
    (value.library.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(descriptor, 57 + descriptorOffset, Hoisted.librarySer2, it)
    }
