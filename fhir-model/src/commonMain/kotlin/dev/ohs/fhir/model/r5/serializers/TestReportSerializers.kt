    ...
    buildClassSerialDescriptor("Assert") {
      ...
      element(
        ...
        listSerialDescriptor(
          lazyDescriptor { TestReport.Setup.Action.Assert.Requirement.serializer().descriptor }
        ),
        isOptional = true,
      )
    }
...
internal object TestReportSetupActionAssertRequirementSerializer :
  ...
  private fun serializeInternal(
    encoder: CompositeEncoder,
    `value`: TestReport.Setup.Action.Assert.Requirement,
  ) {
    (value.id)?.let { encoder.encodeStringElement(descriptor, 0, it) }
    if (value.extension.isNotEmpty())
      encoder.encodeSerializableElement(descriptor, 1, Hoisted.extensionSer, value.extension)
...
internal object TestReportSerializer : KSerializer<TestReport> {
  ...
  internal fun deserializeInternal(
    ...
    return TestReport(
      ...
      testScript = Canonical.of(testScript, _testScript)!!,
