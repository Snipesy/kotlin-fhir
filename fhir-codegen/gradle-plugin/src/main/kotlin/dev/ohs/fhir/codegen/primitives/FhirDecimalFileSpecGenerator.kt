 ...
 * this library's public API) and carries [wire] — the exact lexical form to emit on the wire, which
 * always matches the version-specific FHIR `decimal` regex ([regex], baked in at generation time):
 ...
 * - Values constructed from a number derive a canonical, regex-valid wire form (plain notation when
 *   it fits the regex, otherwise scientific).
 *
...
object FhirDecimalFileSpecGenerator {
  fun generate(packageName: String, regex: String): FileSpec {
    ...
    val regexType = ClassName("kotlin.text", "Regex")
    ...
    val companion =
      TypeSpec.companionObjectBuilder()
        ...
        // Declared first so it is initialized before ZERO/ONE/TWO/TEN, which derive their wire
        // form.
        .addProperty(
          PropertySpec.builder("WIRE_REGEX", regexType)
            .addKdoc("The FHIR `decimal` lexical pattern this FHIR version permits on the wire.")
            .initializer(CodeBlock.of("%T(%S)", regexType, regex))
            .build()
        )
        .addFunction(
          FunSpec.builder("fromString")
            .addKdoc(
              "Parses a wire/lexical decimal, preserving its exact lexical form for round-tripping." +
                " Throws if [string] is not a valid FHIR decimal."
            )
            .addParameter("string", string)
            .returns(fd)
            .addStatement(
              "require(string.matches(WIRE_REGEX)) { %P }",
              "Invalid FHIR decimal: \$string",
            )
        ...
        .addFunction(
          FunSpec.builder("fromBigDecimal")
            .addKdoc(
              "Wraps a [BigDecimal], deriving a canonical wire form that satisfies [WIRE_REGEX]" +
                " (plain notation when it fits, otherwise scientific). Throws if the value cannot" +
                " be represented as a FHIR decimal for this version (e.g. too many digits)."
            )
            .addParameter("bigDecimal", bigDecimal)
            .returns(fd)
            .addStatement("return FhirDecimal(bigDecimal, canonicalWire(bigDecimal))")
            .build()
        )
        .addFunction(
          FunSpec.builder("canonicalWire")
            ...
            .addCode(
              CodeBlock.of(
                ...
                if (plain.matches(WIRE_REGEX)) return plain
                val scientific = bigDecimal.toString()
                if (scientific.matches(WIRE_REGEX)) return scientific
                throw IllegalArgumentException(%P)
                """
                  .trimIndent() + "\n",
                "Value cannot be represented as a FHIR decimal: \$bigDecimal",
              )
            )
            .build()
        )
        ...
        .addFunction(
          FunSpec.builder("parseString")
            ...
            .returns(fd)
            .addStatement(
              "return if (base == 10 && string.matches(WIRE_REGEX)) fromString(string) else " +
      ...
      TypeSpec.classBuilder(fd)
        ...
        .addProperty(
          PropertySpec.builder("wire", string)
            .addKdoc("The exact lexical form to emit on the wire; always matches [WIRE_REGEX].")
            .initializer("wire")
            .build()
        )
        .addType(companion)
